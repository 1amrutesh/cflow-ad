...
def get_args():
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="D",
        help="dataset name: mvtec/stc/video/miad (default: miad)",
    )
    parser.add_argument(
        "-enc",
        "--enc-arch",
...
def main(c):
    ...
    for class_idx, class_name in enumerate(class_names):
        for run in range(runs):
            ...
            #
            result_file = list(
                filter(lambda x: x.startswith(c.model), result_files)
            )
            if len(result_file) == 0:
                raise NotImplementedError(
                    "{} results are not found!".format(c.model)
                )
            elif len(result_file) > 1:
                raise NotImplementedError(
                    "{} duplicate results are found!".format(result_file)
                )
            else:
    ...
    for i, m in enumerate(metrics):
        ...
        for j, class_name in enumerate(class_names):
            print(
                r"{:.2f}\tiny$\pm${:.2f} for class {}".format(
                    np.mean(results[i, j]), np.std(results[i, j]), class_name
                )
            )
        # \tiny$\pm$
