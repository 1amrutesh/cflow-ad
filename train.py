...
def train_meta_epoch(
    c, epoch, loader, encoder, decoders, optimizer, pool_layers, N
):
    P = c.condition_vec
    L = c.pool_layers
    ...
    for sub_epoch in range(c.sub_epochs):
        ...
        for i in range(I):
            ...
            lr = warmup_learning_rate(
                c, epoch, i + sub_epoch * I, I * c.sub_epochs, optimizer
            )
            # sample batch
            try:
...
def test_meta_epoch(c, epoch, loader, encoder, decoders, pool_layers, N):
    ...
    start = time.time()
    with torch.no_grad():
        for i, (image, label, mask) in enumerate(
            tqdm(loader, disable=c.hide_tqdm_bar)
        ):
            # save
            if c.viz:
    ...
    if c.verbose:
        print(
            "Epoch: {:d} \t test_loss: {:.4f} and {:.2f} fps".format(
                epoch, mean_test_loss, fps
            )
        )
    #
...
def train(c):
    ...
    # data
    kwargs = (
        {"num_workers": c.workers, "pin_memory": True} if c.use_cuda else {}
    )
    # task data
    if c.dataset == "mvtec":
        ...
        train_dataset = MIADDataset(c, is_train=True)
        test_dataset = MIADDataset(c, is_train=False)
    else:
        raise NotImplementedError(
            "{} is not supported dataset!".format(c.dataset)
        )
    #
    ...
    for epoch in range(c.meta_epochs):
        if c.action_type == "norm-test" and c.checkpoint:
            ...
            raise NotImplementedError(
                "{} is not supported action type!".format(c.action_type)
            )
        ...
        if save_best_seg_weights and c.action_type != "norm-test":
            save_weights(
                encoder, decoders, c.model, run_date
            ...
            for step in range(max_step):
                ...
                for i in range(len(binary_score_maps)):  # for i th image
                    ...
                    for prop in props:
                        ...
                        cropped_pred_label = binary_score_maps[i][
                    ...
                    # iou (per image level)
                    intersection = (
                        np.logical_and(binary_score_maps[i], gt_mask[i])
                        .astype(np.float32)
                        .sum()
                    )
                    union = (
                        np.logical_or(binary_score_maps[i], gt_mask[i])
                        .astype(np.float32)
                        .sum()
                    )
                    if (
                ...
                fpr = (
                    np.logical_and(gt_masks_neg, binary_score_maps).sum()
                    / gt_masks_neg.sum()
                )
                fprs.append(fpr)
                threds.append(thred)
    ...
    #
    save_results(
        det_roc_obs, seg_roc_obs, seg_pro_obs, c.model, c.class_name, run_date
    )
    # export visualuzations
    if c.viz:
        precision, recall, thresholds = precision_recall_curve(
            gt_label, score_label
        )
        ...
        precision, recall, thresholds = precision_recall_curve(
            gt_mask.flatten(), super_mask.flatten()
        )
        a = 2 * precision * recall
        b = precision + recall
        f1 = np.divide(a, b, out=np.zeros_like(a), where=b != 0)
        ...
        export_test_images(
            c, test_image_list, gt_mask, super_mask, seg_threshold
        )
