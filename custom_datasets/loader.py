...
__all__ = ("MVTecDataset", "StcDataset", "MIADDataset")
...
class StcDataset(Dataset):
    def __init__(self, c, is_train=True):
        assert (
            c.class_name in STC_CLASS_NAMES
        ), "class_name: {}, should be in {}".format(
            c.class_name, STC_CLASS_NAMES
        )
        ...
        if is_train:
            ...
            self.dataset_files = sorted(
                [
                    f
                    for f in os.listdir(self.dataset_vid)
    ...
    def load_dataset_folder(self):
        ...
        for i, img_type in enumerate(img_types):
            ...
            if phase == "test":
                gt_pixel = np.load(
                    "{}.npy".format(os.path.join(gt_pixel_dir, img_type))
                )
                gt_frame = np.load(
                    "{}.npy".format(os.path.join(gt_frame_dir, img_type))
                )
                if i == 0:
                    m = gt_pixel
                    y = gt_frame
                ...
                assert len(x) == len(
                    mask
                ), "number of x and mask should be same"
            else:
...
class MVTecDataset(Dataset):
    def __init__(self, c, is_train=True):
        assert (
            c.class_name in MVTEC_CLASS_NAMES
        ), "class_name: {}, should be in {}".format(
...
class MIADDataset(Dataset):
    def __init__(self, c, is_train=True):
        assert (
            c.class_name in MIAD_CLASS_NAMES
        ), "class_name: {}, should be in {}".format(c.class_name, MIAD_CLASS_NAMES)
        self.dataset_path = c.data_path
        self.class_name = c.class_name
        self.is_train = is_train
        self.cropsize = c.crp_size
    ...
    def load_dataset_folder(self):
        ...
        for class_name in MIAD_CLASS_NAMES:
            img_dir = os.path.join(self.dataset_path, class_name, phase)
            gt_dir = os.path.join(
                self.dataset_path, class_name, "ground_truth"
            )
            ...
            for img_type in img_types:
                ...
                x.extend(good_img_fpath_list)
                y.extend([0] * len(good_img_fpath_list))
                mask.extend([None] * len(good_img_fpath_list))
